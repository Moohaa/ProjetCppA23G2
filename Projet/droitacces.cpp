...
bool droit_acces::Ajouter_droit()
{
       ...
       QSqlQuery Query;
       Query.prepare("INSERT INTO DROIT_ACCES(CODE_DROIT,LIBELLE_DROIT)"
...
model->setQuery("select * from DROIT_ACCES");
...
{
QSqlQuery query;
query.prepare("Delete from DROIT_acces where CODE_DROIT = :CODE_DROIT ");
...
bool droit_acces::modifier_droit(int code,QString libel)
{
    ...
    QSqlQuery qry;
        ...
        qry.prepare("UPDATE DROIT_ACCES set libelle_droit=(?) where CODE_DROIT=(?)");
